import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, unique, int, varchar, char, index, foreignKey, datetime, text, date, tinyint } from "drizzle-orm/mysql-core"
...
(table) => [
	index("fk_favorites_practices_idx").on(table.practiceId),
	index("fk_favorites_providers_idx").on(table.providerId),
	index("fk_favorites_users_idx").on(table.userId),
	primaryKey({ columns: [table.id], name: "user_favorites_id"}),
	unique("id_UNIQUE").on(table.id),
]);
...
export const users = mysqlTable("users", {
	...
	resetTokenExpiration: datetime("reset_token_expiration", { mode: 'string'}),
